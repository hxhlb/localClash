...
func Registry() []Tool {
	tools := []Tool{
		...
		{Name: "restart_runtime", SafetyLevel: ConfirmRequired, Description: "Reload the running Mihomo runtime. MCP defaults to hot reload, which verifies the current config hash against a prior mihomo_config_test attestation before calling the Mihomo API. Use strategy=process_restart for an explicit stop/start process restart."},
	...
	case "restart_runtime":
		return map[string]any{
			...
			"properties": map[string]any{
				...
				"log_file":          map[string]any{"type": "string", "description": "Runtime log file. Defaults to .runtime/mihomo/mihomo.log."},
				"strategy":          map[string]any{"type": "string", "enum": []string{"hot_reload", "process_restart"}, "description": "Restart strategy. Defaults to hot_reload for MCP restart_runtime."},
