...
func Registry() []Tool {
	tools := []Tool{
		...
		{Name: "config_patch_create", SafetyLevel: SafeWrite, Description: "Create a reviewable config patch and candidate Mihomo config from proxy groups, packs, and custom rules. It does not modify active localclash.json or generated/mihomo.yaml; apply the returned patch_id only after review. If mihomo_test fails, do not bypass it with test=false unless the user explicitly accepts skipping Mihomo validation."},
		{Name: "config_render", SafetyLevel: SafeWrite, Description: "Render generated/mihomo.yaml from the current durable localclash.json source of truth, subscription, policy, and runtime profile. Does not read patches and does not start runtime."},
	...
	case "config_status":
		return map[string]any{
			...
			"properties": map[string]any{
				...
				"subscription_config":  map[string]any{"type": "string", "description": "Subscription sources config path. Defaults to localclash-subscriptions.json."},
				"subscription_runtime": map[string]any{"type": "string", "description": "Per-source subscription artifact directory. Defaults to .runtime/subscriptions."},
				"policy":               map[string]any{"type": "string", "description": "Policy YAML path. Defaults to policies/loyalsoldier.json."},
