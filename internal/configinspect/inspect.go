...
type BaseSummary struct {
	...
	RulesCountScope    string                `json:"rules_count_scope,omitempty"`
	RulesTotalCount    int                   `json:"rules_total_count,omitempty"`
...
func InspectBase(opts Options) (BaseResult, error) {
	...
	allRules := stringSlice(config["rules"])
	rules := filterStrings(allRules, overlayRuleLines)
	scope := "all_generated_config"
	rulesCountScope := "all_generated_config_rules"
	note := "Base config is generated by localClash and is not modified through MCP plan tools."
	if len(overlayRuleLines) > 0 {
		scope = "base_excluding_localclash_overlay"
		rulesCountScope = "generated rules after excluding x-localclash.overlay.rules"
		note = "Base summary excludes localClash overlay entries recorded in x-localclash.overlay; rules_total_count is the full generated rules count."
	}
	return BaseResult{
		Config:     path,
		Layer:      "base",
		...
		Summary: BaseSummary{
			...
			RulesCountScope:    rulesCountScope,
			RulesTotalCount:    len(allRules),
